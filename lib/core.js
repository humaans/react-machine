    ...
    create({
      state: (name, ...opts) => {
        ...
        if (typeof opts[0] === 'function') {
          state = createNestedState(name, opts[0])
        } else {
          state = createState(name, ...opts)
        }

        if (stack.length > 0) {
          stack[stack.length - 1].push({ type: 'state', node: state })
        } else {
          machine.states[name] = state
        }
      },
      enter: withStack(createEnter),
      exit: withStack(createExit),
      transition: withStack(createTransition),
      immediate: withStack(createImmediate),
      internal: withStack(createInternal),
    })
  }
 ...
 * Create a state node.
 */
function withStack(createHook) {
  return (...args) => {
    const hook = createHook(...args)
    if (stack.length > 0) {
      stack[stack.length - 1].push(hook)
    }
    return hook
  }
}

function createNestedState(name, create) {
  console.log('    Creating nested state', name)

  stack.push([])
  if (create) create()
  const opts = stack.pop()

  const node = createState(name, ...opts)

  return node
}

function createState(name, ...opts) {
  console.log('    Creating regular state', name)

  if (stack.length > 0) {
    const lastIndex = stack.length - 1
    stack[lastIndex] = stack[lastIndex].filter((o) => !opts.includes(o))
  }
