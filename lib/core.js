...
function applyTransition(machine, context, curr, event, transition) {
  ...
  queueEffects(transition, 'transition', effects, event)
  ...
  if (!transition.internal) {
    if (currNode) {
      for (const exit of currNode.exit) {
        queueEffects(exit, 'exit', effects, event)
      }
    }

    for (const enter of nextNode.enter) {
      queueEffects(enter, 'enter', effects, event)
    }
  }
...
function queueEffects({ effects }, kind, queue, event) {
  for (const effect of effects) {
    queue.push({ run: effect, event, kind })
  }
}
 ...
 */
export function runEffects(effects = [], context, state, send) {
