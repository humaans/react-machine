...
export function transition(machine = {}, context = {}, state = {}, event, { assign } = {}) {
  ...
  if (!state.name && event && event.type === null) {
    ...
    if (nodeNames.length > 0) {
      const initialNodeName = nodeNames[0]
      const initialTransition = createImmediate(initialNodeName)
      return applyTransition(machine, context, state, event, initialTransition)
    }
  }
 ...
 */
function applyTransition(machine, context, curr, event, transition) {
  const next = { ...curr }
  ...
  if (currNode) {
    for (const exit of currNode.exit) {
      queueEffects(exit, effects, event)
    }
  }
  ...
  queueEffects(transition, effects, event, { restart: transition.internal })
  ...
  for (const enter of nextNode.enter) {
    queueEffects(enter, effects, event)
  }

  for (const candidate of nextNode.immediates) {
    if (checkGuards(context, next, event, candidate)) {
      return applyTransition(machine, context, next, event, candidate)
    }
  }
...
function queueEffects({ effects }, queue, event, { restart = false } = {}) {
  for (const effect of effects) {
    queue.push({ ...effect, event, restart })
  }
}
 ...
 * transitioning the machine to the next state node, the caller must clean
 * up all of the running effects, and then run the newly provided effects.
 */
export function applyEffects(runningEffects = [], queuedEffects = [], context, state, send) {
  const nextEffects = []

  const effectsToKeep = {}
  for (const effect of queuedEffects) {
    if (!effect.restart) {
      effectsToKeep[effect.id] = true
    }
  }

  const effectsAlreadyRunning = {}
  for (const effect of runningEffects) {
    if (!effect.restart) {
      effectsAlreadyRunning[effect.id] = true
    }
  }

  for (const effect of runningEffects) {
    if (!effectsToKeep[effect.id]) {
      if (effect.dispose) {
        effect.dispose()
      }
    } else {
  ...
  for (const effect of queuedEffects) {
    if (effectsAlreadyRunning[effect.id]) {
      continue
    }
    ...
    const dispose = effect.run(context, state.data, effect.event, safeSend)
