...
function applyTransition(
  ...
  if (currNode && !transition.internal) {
    effects.exitSet.push(currNode.name)
    for (const exit of currNode.exit) {
      applyReducers(exit, context, next, event)
      queueEffects(exit, effects, event, target)
    }
  }
  ...
  queueEffects(transition, effects, event, target)

  if (!transition.internal) {
    for (const enter of nextNode.enter) {
      applyReducers(enter, context, next, event)
      queueEffects(enter, effects, event, target)
    }
  }
...
function queueEffects({ effects }, eff, event, target) {
  for (const effect of effects) {
    eff.queue.push({ ...effect, event, target })
  }
}
...
export function applyEffects(runningEffects = [], queuedEffects, context, state, send) {
  // if (!queuedEffects) return

  const nextEffects = []

  for (let i = runningEffects.length - 1; i >= 0; i--) {
    const effect = runningEffects[i]
    if (
      queuedEffects.exitSet.includes(effect.target) ||
      queuedEffects.queue.some((eff) => eff.id === effect.id)
    ) {
      if (effect.dispose) {
        effect.dispose()
      }
  ...
  for (const effect of queuedEffects.queue) {
    ...
    if (queuedEffects.exitSet.includes(effect.target)) {
      if (effect.dispose) {
        effect.dispose()
      }
    } else {
